...
def run_automl():
    for file_csv in os.listdir(dataset_dir):
        data=pd.read_csv(dataset_dir+file_csv)
        X_train, X_test, y_train, y_test = train_test_split(df[df.columns[:-1]], df["income"], test_size=0.25)
        automl = AutoML(algorithms=algorithm_list, eval_metric='accuracy', results_path=f"results/{file[:-4]}", explain_level=1, top_models_to_improve=4, random_state=2)
